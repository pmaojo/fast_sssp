...
fn benchmark_scale_free_graphs(csv_file: &mut File) {
    ...
    for &size in &sizes {
        for &m in &edges_per_node {
            println!(
                "\n📈 Testing scale-free graph with {} vertices, {} edges per new node",
                size, m
            );
            ...
            println!(
                "✓ Graph generated with {} vertices and {} edges in {:.2}s",
                graph.vertex_count(),
                graph.edge_count(),
            ...
            writeln!(
                csv_file,
                "scale_free,{},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{}",
                size,
                m,
                result.reachable,
                result.dijkstra_ms,
                result.fast_sssp_ms,
                result.fast_sssp_no_transform_ms,
                result.smart_auto_ms,
                result.smart_adaptive_ms,
                result.winner
            )
            .unwrap();
        }
...
fn benchmark_3d_grid_graphs(csv_file: &mut File) {
    ...
    for &(nx, ny, nz) in &dimensions {
        let size = nx * ny * nz;
        println!(
            "\n📈 Testing 3D grid graph with dimensions {}x{}x{} ({} vertices)",
            nx, ny, nz, size
        );
...
fn benchmark_geometric_graphs(csv_file: &mut File) {
    ...
    for &size in &sizes {
        for &radius in &radii {
            println!(
                "\n📈 Testing 3D geometric graph with {} vertices, radius {}",
                size, radius
            );
...
fn benchmark_algorithms(
    ...
    if fast_sssp_correct {
        println!(
            "✓ FastSSSP with HubSplit correctly found all {} reachable vertices",
            reachable
        );
    } else {
    ...
    let fast_sssp_no_transform_result = fast_sssp_no_transform
    ...
    // Verify correctness
    let fast_sssp_no_transform_correct = verify_results(
        &dijkstra_result.distances,
        &fast_sssp_no_transform_result.distances,
    );
    if fast_sssp_no_transform_correct {
        println!(
            "✓ FastSSSP without transformation correctly found all {} reachable vertices",
            reachable
        );
    } else {
    ...
    // Verify correctness
    let smart_auto_correct =
        verify_results(&dijkstra_result.distances, &smart_auto_result.distances);
    if smart_auto_correct {
        println!(
            "✓ SmartSSSP (Auto) correctly found all {} reachable vertices",
            reachable
        );
    } else {
    ...
    let start = Instant::now();
    let smart_adaptive = SmartSSSP::with_mode(SmartMode::Adaptive).with_stats_collection(true);
    ...
    // Verify correctness
    let smart_adaptive_correct =
        verify_results(&dijkstra_result.distances, &smart_adaptive_result.distances);
    if smart_adaptive_correct {
        println!(
            "✓ SmartSSSP (Adaptive) correctly found all {} reachable vertices",
            reachable
        );
    } else {
