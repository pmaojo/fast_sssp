...
fn main() {
    ...
    let graph_sizes = vec![
        // Small graphs
        1_000, 10_000, // Medium graphs - might start to see benefits
        50_000, // Large graphs - should see definite benefits
        100_000, 200_000, // Very large graphs - if memory allows
        500_000,
    ];
    ...
    for &size in &graph_sizes {
        ...
        println!(
            ...
            graph.vertex_count(),
            (size as f64 * edge_factor) as usize
        );

        // Run benchmarks
        let dijkstra_time = benchmark_algorithm("Dijkstra", &dijkstra, &graph, source);
        let fastssp_default_time =
            benchmark_algorithm("FastSSSP (default)", &fast_sssp_default, &graph, source);
        let fastssp_always_time =
            benchmark_algorithm("FastSSSP (always)", &fast_sssp_always, &graph, source);

        // Store results
        results.push((
            size,
            dijkstra_time,
            fastssp_default_time,
            fastssp_always_time,
        ));
        ...
        println!(
            "Speedup - FastSSSP (default) vs Dijkstra: {:.2}x",
            speedup_default
        );
        println!(
            "Speedup - FastSSSP (always) vs Dijkstra: {:.2}x",
            speedup_always
        );
    }
