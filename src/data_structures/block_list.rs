    ...
    V: Float + Zero + Debug + Copy + PartialOrd + Ord,
{
    ...
    pub fn insert(&mut self, key: K, value: V) {
        ...
        if let Some(old_value) = self.key_values.get(&key) {
            ...
            // Lazy deletion: we don't remove old value from blocks
            // Just update the mapping and insert the new value
            // The old value will be filtered out during pull()
        }
    ...
    pub fn batch_prepend(&mut self, pairs: Vec<(K, V)>) {
        ...
        for (&key, &value) in unique_pairs.iter() {
            if let Some(old_value) = self.key_values.get(&key) {
                ...
                // Lazy deletion: skip removing old value
            }
            self.key_values.insert(key, value);
        }
            ...
            if !pairs_vec.is_empty() {
                let max_value =
                    pairs_vec
                        .iter()
                        .map(|(_, v)| *v)
                        .fold(V::zero(), |a, b| if a > b { a } else { b });
                // Prepend to D0 blocks (insert at beginning)
                self.d0_blocks.insert(
                    0,
                    Block {
                        pairs: pairs_vec,
    ...
    pub fn pull(&mut self, max_count: usize) -> (Vec<K>, V) {
        ...
        // We pull potentially more elements to account for stale ones
        // Heuristic: pull 2x max_count initially, but we might need more
        // Since we are sorting, pulling too many is expensive
        // But pulling too few means we might return empty set
        // Let's stick to max_count for the raw pull, but filter aggressively
        // If we get few valid elements, so be it (BMSSP will loop again)

        // Collect elements from D0 first
        let mut d0_elements = collect_from_blocks(&mut self.d0_blocks, max_count);

        // Then from D1 if needed
        let remaining = max_count.saturating_sub(d0_elements.len());
        ...
        // Filter out stale elements and take up to max_count valid elements
        let mut taken_count = 0;
        let mut last_idx = 0;

        for (i, (key, value)) in all_elements.iter().enumerate() {
            if taken_count >= max_count {
                break;
            }

            // Check if this is the current valid value for the key
            if let Some(&current_val) = self.key_values.get(key) {
                // Using exact equality for floats might be tricky, but here we inserted exact copy
                // PartialOrd should handle it. value <= current_val means valid (since duplicate insert is possible with same value)
                // Actually, duplicate with same value is fine.
                // But if value > current_val, it's stale.
                if *value <= current_val {
                    result.push(*key);
                    // Remove from key_values to mark as processed
                    self.key_values.remove(key);
                    taken_count += 1;
                }
            }
            // If key not in key_values, it was already processed (stale), so skip

            last_idx = i + 1;
        }

        // Find the next bound
        if last_idx < all_elements.len() {
            // Next bound is the value of the first element we didn't take (valid or stale)
            // Even if stale, it provides a safe lower bound for remaining elements
            next_bound = all_elements[last_idx].1;
    ...
    /// Drains all elements from the block list
    pub fn drain_all(&mut self) -> Vec<(K, V)> {
        let mut result = Vec::new();
        // Just extract from key_values as that is the source of truth
        for (k, v) in self.key_values.drain() {
            result.push((k, v));
        }

        // Clear blocks
        self.d0_blocks.clear();
        self.d1_blocks.clear();
        self.d1_blocks.push(Block {
            pairs: Vec::new(),
            upper_bound: self.upper_bound,
        });
