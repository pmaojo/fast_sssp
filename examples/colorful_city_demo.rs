...
use fast_sssp::{Dijkstra, DirectedGraph, FastSSSP, ShortestPathAlgorithm, ShortestPathResult};
...
fn city_to_graph(city: &CityGrid) -> DirectedGraph<OrderedFloat<f64>> {
    ...
    for (name, &(x, y)) in &city.locations {
        if !city.is_walkable(x, y) {
            println!(
                "WARNING: Location '{}' at ({}, {}) is not walkable!",
                name, x, y
            );
            all_walkable = false;
        }
    ...
    for (name, &(x, y)) in &city.locations {
        ...
        if let Some(existing) = location_coords.get(&coord) {
            println!(
                "WARNING: Multiple locations at ({}, {}): '{}' and '{}'",
                x, y, existing, name
            );
        } else {
...
fn visualize_city_with_animated_path(city: &CityGrid, path: Option<&[usize]>, animate: bool) {
    if animate && path.is_some() {
        ...
        for step in 0..=path.unwrap().len() {
            ...
            println!(
                "{}",
                "🏙️  CITY PATHFINDING VISUALIZATION  🏙️"
                    .bright_cyan()
                    .bold()
            );
            println!();
        ...
        println!(
            "\n{} {}",
            "✅".bright_green(),
            "Path complete!".bright_green().bold()
        );
    } else {
...
fn show_welcome_screen() {
    ...
    println!(
        "{}",
        "    FAST SSSP CITY PATHFINDING DEMO    "
            .bright_cyan()
            .bold()
            .on_black()
    );
    ...
    println!(
        "{}",
        "Welcome to the interactive city pathfinding demonstration!".bright_white()
    );
    println!(
        "{}",
        "This demo showcases the O(m log^(2/3) n) Fast SSSP algorithm".bright_yellow()
    );
    println!(
        "{}",
        "finding optimal paths through a city with buildings and obstacles.".bright_yellow()
    );
    println!();
    println!(
        "{} {}",
        "🚀".bright_red(),
        "Features:".bright_white().bold()
    );
...
fn interactive_demo() {
    ...
    loop {
        ...
        println!(
            "{}. {} (morning commute)",
            "1".bright_green(),
            "Home → Work".bright_white()
        );
        println!(
            "{}. {} (lunch break)",
            "2".bright_green(),
            "Work → Cafe".bright_white()
        );
        println!(
            "{}. {} (evening workout)",
            "3".bright_green(),
            "Home → Gym".bright_white()
        );
        println!(
            "{}. {} (weekend stroll)",
            "4".bright_green(),
            "Cafe → Park".bright_white()
        );
        println!(
            "{}. {} (shopping trip)",
            "5".bright_green(),
            "Home → Store".bright_white()
        );
        println!("{}. {}", "6".bright_blue(), "Custom route".bright_white());
        println!(
            "{}. {}",
            "7".bright_magenta(),
            "Performance comparison".bright_white()
        );
        ...
        match input.trim() {
            ...
            "8" => {
                println!(
                    "\n{} {}",
                    "👋".bright_yellow(),
                    "Thanks for exploring the city!".bright_green().bold()
                );
                break;
            }
            _ => {
                println!(
                    "{} {}",
                    "❌".bright_red(),
                    "Invalid choice. Please enter 1-8.".bright_red()
                );
...
fn find_and_display_path(
    ...
    println!(
        "Computing shortest path from {} ({},{}) to {} ({},{})",
        from, fx, fy, to, tx, ty
    );
    ...
    if let Some(dist) = result.distances[target] {
        println!(
            "Path computation complete. Distance to target: {}",
            dist.into_inner()
        );
    } else {
    ...
    if let Some(distance) = result.distances[target] {
        ...
        println!(
            "{} {}",
            "✅".bright_green(),
            "Path found!".bright_green().bold()
        );
        println!(
            "{} {:.2} blocks",
            "📏 Distance:".bright_white(),
            distance.into_inner().to_string().bright_yellow()
        );
        println!(
            "{} {} moves",
            "👣 Steps:".bright_white(),
            (path.len() - 1).to_string().bright_yellow()
        );
        println!(
            "{} {:?}",
            "⚡ Computation time:".bright_white(),
            format!("{:?}", computation_time).bright_cyan()
        );
...
fn performance_comparison(city: &CityGrid, graph: &DirectedGraph<OrderedFloat<f64>>) {
    ...
    println!(
        "{} {:?}",
        "⏱️ Fast SSSP time:".bright_white(),
        format!("{:?}", fast_time).bright_cyan()
    );
    println!(
        "{} {:?}",
        "⏱️ Dijkstra time:".bright_white(),
    ...
    if fast_time < dijkstra_time {
        ...
        println!(
            "{} {}",
            "📝".bright_yellow(),
            "Note: For small graphs, overhead may make Dijkstra appear faster".bright_white()
        );
    }
        ...
        if name != "home" {
            ...
            match (fast_dist, dijkstra_dist) {
                (Some(fd), Some(dd)) => {
                    ...
                    let match_symbol = if (fd_val - dd_val).abs() < 1e-10 {
                        "✅".bright_green()
                    } else {
                        "❌".bright_red()
                    };
                    println!(
                        ...
                        name.bright_white(),
                        match_symbol,
                        fd_val.to_string().bright_yellow(),
                        dd_val.to_string().bright_cyan(),
                        if (fd_val - dd_val).abs() < 1e-10 {
                            "✓".bright_green()
                        } else {
                            "✗".bright_red()
                        }
    ...
    if all_match {
        println!(
            "\n{} {}",
            "🎉".bright_green(),
            "All distances match perfectly!".bright_green().bold()
        );
    }
    ...
    W: Float + Zero + Debug + Copy + Ord,
{
    ...
    while current != result.source {
        ...
        if !visited.insert(current) {
            println!(
                "WARNING: Cycle detected in path reconstruction at vertex {}",
                current
            );
            return None;
        }
