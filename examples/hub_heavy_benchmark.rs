    ...
    W: Clone + PartialEq + std::fmt::Debug + Zero + num_traits::Float,
{
    ...
    if result_reachable == baseline_reachable {
        println!(
            "  ✓ {} correctly found all {} reachable vertices",
            mode_name, baseline_reachable
        );
    } else {
        println!(
            "  ❌ {} found {} reachable vertices, but baseline found {}",
    ...
    for (i, (result_dist, baseline_dist)) in result
        .distances
        .iter()
        .zip(baseline.distances.iter())
        .enumerate()
    {
    ...
    if mismatches == 0 {
        println!(
            "  ✓ {} distances match baseline for all reachable vertices",
            mode_name
        );
    } else {
        let match_percentage = 100.0 * (checked - mismatches) as f64 / checked as f64;
        println!(
            "  ⚠️ {} had {} distance mismatches with baseline ({:.1}% match)",
