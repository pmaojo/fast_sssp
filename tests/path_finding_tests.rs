...
fn test_path_finding_simple_grid() {
    ...
    // Verify distances
    assert!(
        dijkstra_result.distances[target].is_some(),
        "Dijkstra should find a path"
    );
    assert!(
        fast_sssp_result.distances[target].is_some(),
        "FastSSSP should find a path"
    );

    // Verify paths
    let dijkstra_path = <Dijkstra as ShortestPathAlgorithm<
        OrderedFloat<f64>,
        DirectedGraph<OrderedFloat<f64>>,
    >>::get_path(&dijkstra, &dijkstra_result, target);
    let fast_sssp_path = <FastSSSP as ShortestPathAlgorithm<
        OrderedFloat<f64>,
        DirectedGraph<OrderedFloat<f64>>,
    >>::get_path(&fast_sssp, &fast_sssp_result, target);
    ...
    assert_eq!(
        dijkstra_path[dijkstra_path.len() - 1],
        target,
    ...
    assert_eq!(
        fast_sssp_path[fast_sssp_path.len() - 1],
        target,
        "Path should end at target"
    );
}
...
fn test_path_finding_with_obstacles() {
    ...
    assert!(
        dijkstra_result.distances[target].is_some(),
        "Dijkstra should find a path around obstacles"
    );
    assert!(
        fast_sssp_result.distances[target].is_some(),
        "FastSSSP should find a path around obstacles"
    );
    ...
    assert!(
        ...
        "Dijkstra should construct a path around obstacles"
    );
    assert!(
        fast_sssp_path.is_some(),
        "FastSSSP should construct a path around obstacles"
    );
    ...
    for (from_name, &(fx, fy)) in &locations {
        for (to_name, &(tx, ty)) in &locations {
            if from_name != to_name {
                ...
                assert!(
                    result.distances[target].is_some(),
                    "Should find a path from {} to {}",
                    from_name,
                    to_name
                );

                let path = <FastSSSP as ShortestPathAlgorithm<
                    OrderedFloat<f64>,
                    DirectedGraph<OrderedFloat<f64>>,
                >>::get_path(&fast_sssp, &result, target);
                assert!(
                    path.is_some(),
                    "Should construct a path from {} to {}",
                    from_name,
                    to_name
                );
