...
use crate::algorithm::fast_sssp::{DegreeMode, FastSSSP};
...
impl SmartSSSP {
    ...
    where
        ...
        G: Graph<W>
            + Clone
            + crate::graph::traits::MutableGraph<W>
            + crate::graph::traits::ToConstantDegree<W>,
    {
        let n = graph.vertex_count();
        ...
        if reach_ratio < self.reachable_threshold {
            if self.verbose {
                println!(
                    ...
                    reach,
                    (self.reachable_threshold * n as f64) as usize
                );
            }
            return self.run_dijkstra(graph, source);
        }
        ...
        if max_degree > self.max_degree_threshold {
            if self.verbose {
                println!(
                    "High maximum degree ({}), using FastSSSP with HubSplit",
                    max_degree
                );
            }
            ...
            if self.verbose {
                println!(
                    "Low maximum degree ({}), using simplified FastSSSP",
                    max_degree
                );
            }
    ...
    fn run_fast_sssp_with_hubsplit<W, G>(
        &self,
        graph: &G,
        source: usize,
    ...
    fn run_simplified_fast_sssp<W, G>(
        &self,
        graph: &G,
        source: usize,
    ) -> Result<ShortestPathResult<W>>
            ...
            + crate::graph::traits::ToConstantDegree<W>,
    {
        ...
        if self.collect_stats {
            ...
            if dijkstra_ms <= fast_sssp_ms && dijkstra_ms <= simple_ms {
                ...
                if self.verbose {
                    println!(
                        "  Winner: FastSSSP with HubSplit (speedup: {:.2}x)",
                        speedup
                    );
                }
