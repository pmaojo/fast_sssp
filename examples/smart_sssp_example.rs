...
fn compare_smart_modes() {
    ...
    println!("⏱️  SmartSSSP (SimpleFastSSSP) time: {:.2}ms", smart_simple_ms);
    ...
    println!("⏱️  SmartSSSP (ForceFastSSSP) time: {:.2}ms", smart_force_ms);
...
fn run_comparison(graph: &DirectedGraph<OrderedFloat<f64>>, source: usize) {
    ...
    println!("⏱️  Time: {:.2}ms", fast_sssp_no_transform_time.as_secs_f64() * 1000.0);
