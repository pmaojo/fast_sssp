use fast_sssp::{
    ...
    algorithm::fast_sssp::{DegreeMode, FastSSSP},
    ...
    G: Graph<Weight> + Clone + MutableGraph<Weight> + ToConstantDegree<Weight>,
{
    ...
    println!(
        "  Time: {:.2?}, Reachable vertices: {}",
        dijkstra_time, dijkstra_reachable
    );
    ...
    let start = Instant::now();
    let classic_result = fast_sssp_classic
    ...
    println!(
        "  Time: {:.2?}, Reachable vertices: {}",
        classic_time, classic_reachable
    );
    println!(
        "  Slowdown vs Dijkstra: {:.2}x",
        classic_time.as_secs_f64() / dijkstra_time.as_secs_f64()
    );
    ...
    let start = Instant::now();
    let auto_result = fast_sssp_auto
        .compute_shortest_paths(graph, source)
        .unwrap();
    ...
    println!(
        "  Time: {:.2?}, Reachable vertices: {}",
        auto_time, auto_reachable
    );
    println!(
        "  Speedup vs classic: {:.2}x",
        classic_time.as_secs_f64() / auto_time.as_secs_f64()
    );
    println!(
        "  Slowdown vs Dijkstra: {:.2}x",
        auto_time.as_secs_f64() / dijkstra_time.as_secs_f64()
    );
    ...
    let start = Instant::now();
    let none_result = fast_sssp_none
        .compute_shortest_paths(graph, source)
        .unwrap();
    ...
    println!(
        "  Time: {:.2?}, Reachable vertices: {}",
        none_time, none_reachable
    );
    println!(
        "  Speedup vs classic: {:.2}x",
        classic_time.as_secs_f64() / none_time.as_secs_f64()
    );
    println!(
        "  Slowdown vs Dijkstra: {:.2}x",
        none_time.as_secs_f64() / dijkstra_time.as_secs_f64()
    );
...
fn count_reachable<W: ordered_float::Float + std::fmt::Debug + Copy + Zero>(
...
fn verify_results<W: std::cmp::PartialEq + std::fmt::Debug + Copy + ordered_float::Float>(
    ...
    if baseline_reachable != result_reachable {
        println!(
            "  WARNING: {} mode found {} reachable vertices, but baseline found {}",
            mode_name, result_reachable, baseline_reachable
        );
    } else {
        println!(
            "  ✓ {} mode correctly found all {} reachable vertices",
    ...
    if mismatches > 0 {
        println!(
            "  WARNING: {} mode had {} distance mismatches with baseline",
            mode_name, mismatches
        );
    } else {
        println!(
            "  ✓ {} mode distances match baseline for all reachable vertices",
            mode_name
        );
    }
}
